...
os.environ["DJANGO_SETTINGS_MODULE"] = "book_sys.settings"
django.setup()
...
class UserCf:
    ...
    def nearest_user(self, username, n=1):
        ...
        # 最相似的N个用户
        print("closest mysite:", closest_distance[:n])
...
def recommend_by_user_id(user_id):
    ...
    if current_user.rate_set.count() == 0:
        book_list = Book.objects.all().order_by("-collector")[:15]
        return book_list
    ...
    print("this is all mysite:", all_user)
    ...
    if not good_list:
        ...
        book_list = Book.objects.all().order_by("-collector")[:15]
        return book_list
    book_list = Book.objects.filter(id__in=good_list).order_by("-collector")[:15]
    return book_list
